...
def forward(
    ...
    pos_embeds = fast_pos_embed_interpolate(model.visual, image_grid_thw)
...
def fast_pos_embed_interpolate(emb, grid_thw):
    grid_thw_list = grid_thw.tolist()
    grid_ts = [row[0] for row in grid_thw_list]
    grid_hs = [row[1] for row in grid_thw_list]
    grid_ws = [row[2] for row in grid_thw_list]
    device = emb.pos_embed.weight.device

    idx_list = [[] for _ in range(4)]
    weight_list = [[] for _ in range(4)]

    for t, h, w in grid_thw_list:
        h_idxs = torch.linspace(0, emb.num_grid_per_side - 1, h)
        w_idxs = torch.linspace(0, emb.num_grid_per_side - 1, w)

        h_idxs_floor = h_idxs.int()
        w_idxs_floor = w_idxs.int()
        h_idxs_ceil = (h_idxs.int() + 1).clip(max=emb.num_grid_per_side - 1)
        w_idxs_ceil = (w_idxs.int() + 1).clip(max=emb.num_grid_per_side - 1)

        dh = h_idxs - h_idxs_floor
        dw = w_idxs - w_idxs_floor

        base_h = h_idxs_floor * emb.num_grid_per_side
        base_h_ceil = h_idxs_ceil * emb.num_grid_per_side
    ...
    weight_tensor = torch.tensor(weight_list, dtype=emb.pos_embed.weight.dtype, device=device)
    pos_embeds = emb.pos_embed(idx_tensor).to(device) * weight_tensor[:, :, None]
    patch_pos_embeds = pos_embeds[0] + pos_embeds[1] + pos_embeds[2] + pos_embeds[3]

    patch_pos_embeds = patch_pos_embeds.split([h * w for h, w in zip(grid_hs, grid_ws)])

    patch_pos_embeds_permute = []
    merge_size = emb.config.spatial_merge_size
